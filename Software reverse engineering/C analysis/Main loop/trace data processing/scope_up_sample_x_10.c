...
void scope_up_sample_x_10(uint16 * buffer, uint offset, uint count)

{
  ...
  //Copy the data and make room for the in between samples
  if (uVar15 != 0)
  {
  ...
  //Fill in the gaps with 1/10 steps
  if (0 < (int)(uVar15 - 1))
  {
    do
    {
  ...
  //This is unclear on what is going on

  //Third bit
  uVar17 = uVar15 - 1;  //Do one sample less
  ...
  if (0 < (int)(uVar15 - 2))  //Two samples less
  {
    do
  ...
  if (count != 0)
  {
    ...
    //Average the samples from the two interpolations. Only problem is that they start at a wrong address for the second data???? Data is written starting from 0x801B8B74
    while (uVar17 != 0)
    {
  ...
  if (0 < (int)uVar17)
  {
    ...
    if ((uVar17 & 1) != 0)
    {
      ...
      *puVar8 = *puVar12;                                 //0x8019D5EE ==> 0x801B8B6E  Seems they start on the second sample of the previous processed data
    }
  ...
  if (0 < (int)uVar9)
  {
    ...
    do
    {
      ...
      {
        uVar16 = uVar16 - uVar2;
        puVar11[1] = uVar2 + (ushort)((uint)uVar16 * DAT_8001360c >> 0x12);                                //0x0000CCCD

...
void FUN_800253e8(void)
{
  ...
  if (*(byte *)(DAT_80025460 + 10) < 9)   //Time base setting. Memory address 0x8019D5AA holds some global state that is used in many functions. The emulator currently has 0x0D in it. Not sure what will make it change
  {                                       //This section is not used in the emulator yet
    iVar13 = 2000;                        //Not realy used as such.

    fpga_write_cmd('\r');     //0x0D
    fpga_write_data('\0');
    fpga_write_data('\0');
    ...
    //Next bit is an other function??? It is jumped to with lr reloaded from stack
    ...
    delay_2(0x28);  //Delay based on timer ticks

...
void SG_RotaryDialSetup(tagXlibContext *xc, tagSG_RotaryDial *rotarydial)
{
  ...
  mouse->move = NULL;
  mouse->down = SG_RotaryDialDown;
  mouse->up   = SG_RotaryDialUp;
  
  //Draw the rotary dial itself
  SG_RotaryDialDraw(rotarydial);
}
...
{
  tagSG_RotaryDial *rotarydial = event->data;
...
void SG_RotaryDialDown(MouseEvent *event)
{
  ...
  SG_RotaryDialDrawPosition(event, False);

  //Set the pointer for handling the moving
  rotarydial->mouse.move = SG_RotaryDialMove;
}
...
void SG_RotaryDialUp(MouseEvent *event)
{
  ...
  SG_RotaryDialDrawPosition(event, True);

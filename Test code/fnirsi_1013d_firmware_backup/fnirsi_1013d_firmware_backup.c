...
#include "fnirsi_1013d_firmware_backup.h"
...
int main(void)
{
  ...
  //Setup the touch panel interface and read the configuration for saving
  tp_i2c_setup();
  ...
  status = save_tp_config(250);
  
  //Check if touch panel save succeeded
  if(status == 0)
  {
  ...
  //Check if this scope has a touch panel with more sens lines then standard
  if(tp_config_data[29] > 0x0A)
  {
    //Display special touch panel text in red
    display_set_fg_color(0x00FF0000);
    display_text(320, 250, "!! SPECIAL TOUCH PANEL DETECTED !!");
  }
  ...
  while(1)
  {
    ...
    if(tp_config_data[29] > 0x0A)
    {
      //Blink the upload text
      display_set_fg_color(0x00FF0000);
      display_text(33, 200, "!! SPECIAL TOUCH PANEL DETECTED, PLEASE UPLOAD ON EEVBLOG !!");
      ...
      display_fill_rect(32, 199, 736, 22);
...
void draw_startup_screen(void)
{
  ...
  display_text(165, 150, "FNIRSI-1013D FIRMWARE BACKUP");
...
int32 save_tp_config(uint32 ypos)
{
  ...
  f_unlink( "FNIRSI_1013D_tp_config.bin");
  
  //Create a file for the touch panel configuration. Fails if it already exists
  if(f_open(&fp, "FNIRSI_1013D_tp_config.bin", FA_CREATE_NEW | FA_WRITE | FA_READ) != FR_OK)
  {
    //Error message in red
    display_set_fg_color(0x00FF0000);
    display_text(20, ypos, "ERROR CREATING TP_CONFIG FILE");

    //Signal failure
    status = -1;
  }
  //Write the touch panel configuration to the sd card  
  else if(f_write(&fp, tp_config_data, sizeof(tp_config_data), 0) != FR_OK)
  {
    //Error message in red
    display_set_fg_color(0x00FF0000);
    display_text(20, ypos, "ERROR WRITING TP_CONFIG FILE");

    //Signal failure
    status = -1;
  }
  
  //Even if f_open failed it is ok to close the file
  f_close(&fp);

  if(status == 0)
  {
    //Success message in preset color
    display_text(20, ypos, "TP_CONFIG FILE WRITTEN");
  }

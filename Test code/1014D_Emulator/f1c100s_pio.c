...
#define CS_WAIT_ADDRESS        0
#define CS_RECEIVE_DATA        1
#define CS_SEND_DATA           2
...
#define I2C_SDA_BIT            0x01
#define I2C_SCL_BIT            0x02

//----------------------------------------------------------------------------------------------------------------------------------
void PortAHandler(F1C100S_PIO_PORT *registers,  u_int32_t mode)
{
  CLOCK_SYNTHESIZER_DATA *csd = (CLOCK_SYNTHESIZER_DATA *)registers->portdata;
  u_int8_t address;
  ...
  if(csd)
  {
    ...
    if(mode == PORT_WRITE)
    {
      ...
      if((registers->data.m_8bit[0] & I2C_SCL_BIT) && ((csd->prev_port_data & I2C_SCL_BIT) == 0))
      {
        //Handle according to the current i2c state
        switch(csd->i2c_state)
        {
          case I2C_STATE_SND_BITS:
            //Set SDA with the bit to send (MSB first)
            if(csd->i2c_currentbyte & 0x80)
            {
              //Set SDA
            ...
            //Select next bit
            csd->i2c_currentbyte <<= 1;

            //Signal another bit is send
            csd->i2c_currentbit++;

            //Check if it was the last bit
            if(csd->i2c_currentbit >= I2C_NUM_BITS)
            {
              //If so switch to receiving the ack
              csd->i2c_state = I2C_STATE_RCV_ACK;

              //Select the next byte to send
              csd->i2c_currentbyte = 0;
              csd->i2c_currentbit = 0;
            }              
            break;

          case I2C_STATE_RCV_BITS:
            //Get the bit from SDA and put it in the current byte
            csd->i2c_currentbyte = (csd->i2c_currentbyte << 1) | ((registers->data.m_8bit[0] & I2C_SDA_BIT) != 0);
            ...
            if(csd->i2c_currentbit >= I2C_NUM_BITS)
            {
              ...
              switch(csd->cs_state)
              {
                case CS_WAIT_ADDRESS:
                  //Check if received byte matches the device address
                  address = csd->i2c_currentbyte >> 1; 
                  if(address == 0x60)
                  {
                    //Write the data to the file if it is open
                    if(csd->trace_file)
                    {
                      fprintf(csd->trace_file, "0x%02X ", csd->i2c_currentbyte);
                    }
                    
                    //Check on the data mode for further action
                    csd->cs_mode = csd->i2c_currentbyte & 0x01;

                    if(csd->cs_mode == I2C_MODE_WRITE)
                    {
                      //For write the next two bytes are the register address
                      csd->cs_state = CS_RECEIVE_DATA;
                    }
                    else
                    {
                      //For a read data needs to be returned
                      csd->cs_state = CS_SEND_DATA;
                    }

                    //Switch to set ack state to show the device is present (The scope code does not use this)
                    csd->i2c_state = I2C_STATE_SND_ACK;
                  }
                  else
                  {
                    //Switch to skip ack state since the data is not for this device
                    csd->i2c_state = I2C_STATE_SKIP_ACK;
                  }
                  break;

                case CS_RECEIVE_DATA:
                  //Write the data to the file if it is open
                  if(csd->trace_file)
                  {
                    fprintf(csd->trace_file, "0x%02X ", csd->i2c_currentbyte);
          ...
          default:
            ...
            csd->i2c_state = I2C_STATE_IDLE;
            break;
        }
      }
      //On rising or falling edge of the SDA pin
      else if((registers->data.m_8bit[0] & I2C_SDA_BIT) != (csd->prev_port_data & I2C_SDA_BIT))
      {
        ...
        if(registers->data.m_8bit[0] & I2C_SCL_BIT)
        {
          ...
          else
          {
            ...
            csd->cs_state = CS_WAIT_ADDRESS;
          }
        }
      }

      //store current as previous
      csd->prev_port_data = registers->data.m_8bit[0];
    }
  }  
}
...
const u_int8_t have_trigger = 0x01;

const u_int8_t fpga_version[2] = { 0x14, 0x32 };

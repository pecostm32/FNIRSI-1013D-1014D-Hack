...
void RotaryDialSetup(tagXlibContext *xc, tagRotaryDial *rotarydial)
{
  ...
  tagRotaryDialSettings *settings = rotarydial->settings;
  
  //The cap is shifted left and up to create 3D look
  //Calculate bounding box coordinates based on cap of the rotary dial
  int xo = rotarydial->xpos - settings->xdisplacement;
  int yo = rotarydial->ypos - settings->ydisplacement;
  int l  = xo;
  int r  = xo + settings->size;
  int t  = yo;
  int b  = yo + settings->size;
...
void RotaryDialDraw(tagRotaryDial *rotarydial)
{
  ...
  double r  = settings->size / 2.0;
  double r1 = settings->ridgeradius;
  double r2 = r1 + settings->rigdelength;
  
  //Calculate the coordinates for drawing the body, cap and the ring on the cap
  int x  = CalcCoordX(0);
  int y  = CalcCoordY(0);
  int x1 = CalcCoordX(settings->xdisplacement);
  int y1 = CalcCoordY(settings->ydisplacement);
  int x2 = CalcCoordX(settings->ringxdisplacement);
  int y2 = CalcCoordY(settings->ringydisplacement);
  
  int d  = settings->size * xc->scaler;
  int dr = settings->size * 0.7 * xc->scaler;
  ...
  double ox = (settings->ridgewidth * s1) / 2;
  double oy = (settings->ridgewidth * c1) / 2;
  ...
  XSetForeground(xc->display, xc->gc, settings->ridgemiddlecolor);
  ...
  XSetForeground(xc->display, xc->gc, settings->ridgebottomcolor);
  ...
  XSetForeground(xc->display, xc->gc, settings->ridgetopcolor);
  XDrawSegments(xc->display, xc->win, xc->gc, lines, 2);
}
...
void RotaryDialDrawPosition(MouseEvent *event, Bool deltacheck)
{
  ...
  double r  = rotarydial->settings->size / 2.0;

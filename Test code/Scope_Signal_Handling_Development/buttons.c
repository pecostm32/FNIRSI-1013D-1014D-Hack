...
void ButtonInternalDrawArrow(tagXlibContext *xc, int xpos, int ypos, int size, int fillcolor, int linecolor, int orientation);
...
void ButtonDrawRoundedRect(tagButton *button)
{
  ...
  FillRect(xc, button->xpos - 2, button->ypos - 2, button->width + 4, button->height + 4, ButtonPanelColor);
  
  //Draw the panel cutout
  DrawFillRoundedRect(xc, button->xpos, button->ypos, 6, button->width, button->height, ButtonCutoutColor, ButtonCutoutColor, 1);

  //Check if up state active
  if(button->state == 0)
  {
    c = ButtonDownBodyColor;
  }
  else
  {
    c = ButtonUpBodyColor;
  ...
  //Draw the down body
  DrawFillRoundedRect(xc, x, y, 6, w, h, c, ButtonLineColor, 1);
  
  //Check if up state active
  if(button->state == 0)
  {
    //If so draw the up body
    DrawFillRoundedRect(xc, button->xpos, button->ypos, 6, w, h, ButtonUpBodyColor, ButtonLineColor, 1);
...
void ButtonDrawArrow(tagButton *button)
{
  ...
  //Clear the panel before redrawing
  FillRect(xc, button->xpos - 2, button->ypos - 2, button->width + 4, button->width + 4, ButtonPanelColor);

  //Draw the panel cutout
  ButtonInternalDrawArrow(xc, button->xpos, button->ypos, button->width, ButtonCutoutColor, ButtonCutoutColor, button->type);
  ...
  //Draw the down body
  ButtonInternalDrawArrow(xc, x, y, s, c, ButtonLineColor, button->type);
  
  //Check if up state active
  if(button->state == 0)
  {
    //If so draw the up body
    ButtonInternalDrawArrow(xc, button->xpos, button->ypos, s, ButtonUpBodyColor, ButtonLineColor, button->type);
  }  
}

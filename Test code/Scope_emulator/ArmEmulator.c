...
int arm_emulator_main(void)
{
  ...
  //Create atoms for the control led and display messages
 arm_msg_id = XInternAtom(display, "SCOPE_DISPLAY_MESSAGE", False); 
...
//          LedDisplaySetBCD(&leddisplays[event.xclient.data.l[0]], event.xclient.data.l[1]);
  ...
  //Stop the led blink thread from running
//  stopledblink();

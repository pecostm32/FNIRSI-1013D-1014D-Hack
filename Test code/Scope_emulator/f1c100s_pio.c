...
void PortEHandler(F1C100S_PIO_PORT *registers,  u_int32_t mode)
{
  ...
  //Trace data needs to be formatted
  //command: 0x0D  write  length: 1500  data: 0x00, 0x00, 0x00, 0x27
  //wc: 0x20  rd(1500):   0x10, 0x12, ........ 0x32
  
  //This means that the number of data reads or writes needs to be counted. This is tricky because it will only be clear on write of a next command
  //So signaling is needed and buffering of the bytes for either read or write
  //Needs some error detection when a write is interrupted with a read or vise versa Although this does not seem to happen
          ...
          case FPGA_COMMAND_WRITE:
            ...
            switch(pd->current_command)
            {
              ...
              case 0x21:  //Channel 2
                ...
                //The scope code does not display data if the full buffer is returned. No idea why yet.
                pd->read_count = 1500;
                pd->read_ptr = scope_ch2_data;
                
//                pd->cmd0x21data[0] = 0x00;
//                pd->cmd0x21data[1] = 0x00;  //0xE8 is to high
                
//                pd->read_ptr = pd->cmd0x21data;
                break;

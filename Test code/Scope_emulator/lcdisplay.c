//----------------------------------------------------------------------------------------------------------------------------------

#include "lcdisplay.h"

//A digit has an empty row and column extra
#define LcDisplayDigitWidth    12
#define LcDisplayDigitHeight   18

#define LcDisplayDigitOffset    6

//The actual dots within a character (2x2 pixels per dot)
#define LcDisplayCharWidth     10
#define LcDisplayCharHeight    16

#define LcDisplayDotSize        2

#define CalcScaled(s) ((s) * xc->scaler)
#define CalcCoord(c)  (BORDER_SIZE + ((c) * xc->scaler))
#define CalcCoordX(x) (BORDER_SIZE + ((display->xpos + (x)) * xc->scaler))
#define CalcCoordY(y) (BORDER_SIZE + ((display->ypos + (y)) * xc->scaler))

//----------------------------------------------------------------------------------------------------------------------------------

void LcdDisplaySetup(tagXlibContext *xc, tagLcDisplay *display)
{
  display->xc = xc;
  
  LcdDisplayDraw(display);
}

//----------------------------------------------------------------------------------------------------------------------------------

void LcdDisplayText(tagLcDisplay *display, int col, int row, char *text)
{
  if((col >= display->chars) || (row >= display->lines))
    return;
  
  int x = display->xpos + (col * LcDisplayDigitWidth) + LcDisplayDigitOffset;
  int y = display->ypos + (row * LcDisplayDigitHeight) + LcDisplayDigitOffset;
  int w = display->xpos + (display->chars * LcDisplayDigitWidth) + LcDisplayDigitOffset;
  int h = display->ypos + (display->lines * LcDisplayDigitHeight) + LcDisplayDigitOffset;
  
  //Display all the characters
  while(*text)
  {
    //Display the current character
    LcdDisplayDrawChar(display, x, y, *text);
    
    //Select next character
    text++;
    
    //Point to next column on the display
    x += LcDisplayDigitWidth;
    
    //Check if need to overflow onto next row
    if(x > w)
    {
      //Start on first column
      x = display->xpos + LcDisplayDigitOffset;
      
      //Skip to next row
      y += LcDisplayDigitHeight;
      
      //Check if it is still on displayable surface
      if(y > h)
        return;
    }
  }
}

//----------------------------------------------------------------------------------------------------------------------------------

void LcdDisplayDraw(tagLcDisplay *display)
{
  tagXlibContext *xc = display->xc;
  XPoint points[3];
  
  int l,c;
  
  int w = (display->chars * LcDisplayDigitWidth) + (LcDisplayDigitOffset * 2);
  int h = (display->lines * LcDisplayDigitHeight) + (LcDisplayDigitOffset * 2);
  int x = display->xpos;
  int y = display->ypos;
  
  //Setup points for top left edge lines
  points[0].x = CalcCoordX(w);
  points[0].y = CalcCoordY(0);
  points[1].x = CalcCoordX(0);
  points[1].y = CalcCoordY(0);
  points[2].x = CalcCoordX(0);
  points[2].y = CalcCoordY(h);
  
  //Draw them
  DrawLines(xc, points, 3, display->tledgecolor, 2);

  //Setup points for bottom right edge lines
  points[0].x = CalcCoordX(w);
  points[0].y = CalcCoordY(0);
  points[1].x = CalcCoordX(w);
  points[1].y = CalcCoordY(h);
  points[2].x = CalcCoordX(0);
  points[2].y = CalcCoordY(h);
  
  //Draw them
  DrawLines(xc, points, 3, display->bredgecolor, 2);
  
  w -= 2;
  h -= 2;
  
  //Fill in the background
  FillRect(xc, x + 1, y + 1, CalcScaled(w), CalcScaled(h), display->bgcolor);
  
  //Move to the first character
  x += LcDisplayDigitOffset;
  y += LcDisplayDigitOffset;
  
  w = CalcScaled(LcDisplayCharWidth);
  h = CalcScaled(LcDisplayCharHeight);
  
  for(l=0;l<display->lines;l++)
  {
    for(c=0;c<display->chars;c++)
    {
      FillRect(xc, x, y, w, h, display->offcolor);

      x += LcDisplayDigitWidth;
    }

    x  = display->xpos + 6;
    y += LcDisplayDigitHeight;
  }
}

//----------------------------------------------------------------------------------------------------------------------------------
// Bit patterns for drawing the characters
//----------------------------------------------------------------------------------------------------------------------------------
unsigned char CharacterMap[256][8] =
{
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x00
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x01
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x02
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x03
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x04
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x05
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x06
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x07
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x08
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x09
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x0A
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x0B
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x0C
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x0D
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x0E
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x0F
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x10
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x11
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x12
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x13
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x14
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x15
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x16
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x17
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x18
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x19
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x1A
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x1B
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x1C
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x1D
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x1E
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x1F
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x20
   { 0x20, 0x20, 0x20, 0x20, 0x00 ,0x00, 0x20, 0x00 },    //0x21
   { 0x50, 0x50, 0x50, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x22
   { 0x50, 0x50, 0xF8, 0x50, 0xF8 ,0x50, 0x50, 0x00 },    //0x23
   { 0x20, 0x78, 0xA0, 0x70, 0x28 ,0xF0, 0x20, 0x00 },    //0x24
   { 0xC0, 0xC8, 0x10, 0x20, 0x40 ,0x98, 0x18, 0x00 },    //0x25
   { 0x60, 0x90, 0xA0, 0x40, 0xA8 ,0x90, 0x68, 0x00 },    //0x26
   { 0x60, 0x20, 0x40, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x27
   { 0x10, 0x20, 0x40, 0x40, 0x40 ,0x20, 0x10, 0x00 },    //0x28
   { 0x40, 0x20, 0x10, 0x10, 0x10 ,0x20, 0x40, 0x00 },    //0x29
   { 0x00, 0x20, 0xA8, 0x70, 0xA8 ,0x20, 0x00, 0x00 },    //0x2A
   { 0x00, 0x20, 0x20, 0xF8, 0x20 ,0x20, 0x00, 0x00 },    //0x2B
   { 0x00, 0x00, 0x00, 0x00, 0x60 ,0x20, 0x40, 0x00 },    //0x2C
   { 0x00, 0x00, 0x00, 0xF8, 0x00 ,0x00, 0x00, 0x00 },    //0x2D
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x60, 0x60, 0x00 },    //0x2E
   { 0x00, 0x08, 0x10, 0x20, 0x40 ,0x80, 0x00, 0x00 },    //0x2F
   { 0x70, 0x88, 0x98, 0xA8, 0xC8 ,0x88, 0x70, 0x00 },    //0x30
   { 0x20, 0x60, 0x20, 0x20, 0x20 ,0x20, 0x70, 0x00 },    //0x31
   { 0x70, 0x88, 0x08, 0x10, 0x20 ,0x40, 0xF8, 0x00 },    //0x32
   { 0xF8, 0x10, 0x20, 0x10, 0x08 ,0x88, 0x70, 0x00 },    //0x33
   { 0x10, 0x30, 0x50, 0x90, 0xF8 ,0x10, 0x10, 0x00 },    //0x34
   { 0xF8, 0x80, 0xF0, 0x08, 0x08 ,0x88, 0x70, 0x00 },    //0x35
   { 0x30, 0x40, 0x80, 0xF0, 0x88 ,0x88, 0x70, 0x00 },    //0x36
   { 0xF8, 0x08, 0x10, 0x20, 0x40 ,0x40, 0x40, 0x00 },    //0x37
   { 0x70, 0x88, 0x88, 0x70, 0x88 ,0x88, 0x70, 0x00 },    //0x38
   { 0x70, 0x88, 0x88, 0x78, 0x08 ,0x10, 0x60, 0x00 },    //0x39
   { 0x00, 0x60, 0x60, 0x00, 0x60 ,0x60, 0x00, 0x00 },    //0x3A
   { 0x00, 0x60, 0x60, 0x00, 0x60 ,0x20, 0x40, 0x00 },    //0x3B
   { 0x10, 0x20, 0x40, 0x80, 0x40 ,0x20, 0x10, 0x00 },    //0x3C
   { 0x00, 0x00, 0xF8, 0x00, 0xF8 ,0x00, 0x00, 0x00 },    //0x3D
   { 0x80, 0x40, 0x20, 0x10, 0x20 ,0x40, 0x80, 0x00 },    //0x3E
   { 0x70, 0x88, 0x08, 0x10, 0x20 ,0x00, 0x20, 0x00 },    //0x3F
   { 0x70, 0x88, 0x08, 0x68, 0xA8 ,0xA8, 0x70, 0x00 },    //0x40
   { 0x70, 0x88, 0x88, 0x88, 0xF8 ,0x88, 0x88, 0x00 },    //0x41
   { 0xF0, 0x88, 0x88, 0xF0, 0x88 ,0x88, 0xF0, 0x00 },    //0x42
   { 0x70, 0x88, 0x80, 0x80, 0x80 ,0x88, 0x70, 0x00 },    //0x43
   { 0xE0, 0x90, 0x88, 0x88, 0x88 ,0x90, 0xE0, 0x00 },    //0x44
   { 0xF8, 0x80, 0x80, 0xF0, 0x80 ,0x80, 0xF8, 0x00 },    //0x45
   { 0xF8, 0x80, 0x80, 0xF0, 0x80 ,0x80, 0x80, 0x00 },    //0x46
   { 0x70, 0x88, 0x80, 0xB8, 0x88 ,0x88, 0x78, 0x00 },    //0x47
   { 0x88, 0x88, 0x88, 0xF8, 0x88 ,0x88, 0x88, 0x00 },    //0x48
   { 0x70, 0x20, 0x20, 0x20, 0x20 ,0x20, 0x70, 0x00 },    //0x49
   { 0x38, 0x10, 0x10, 0x10, 0x10 ,0x90, 0x60, 0x00 },    //0x4A
   { 0x88, 0x90, 0xA0, 0xC0, 0xA0 ,0x90, 0x88, 0x00 },    //0x4B
   { 0x80, 0x80, 0x80, 0x80, 0x80 ,0x80, 0xF8, 0x00 },    //0x4C
   { 0x88, 0xD8, 0xA8, 0xA8, 0x88 ,0x88, 0x88, 0x00 },    //0x4D
   { 0x88, 0x88, 0xC8, 0xA8, 0x98 ,0x88, 0x88, 0x00 },    //0x4E
   { 0x70, 0x88, 0x88, 0x88, 0x88 ,0x88, 0x70, 0x00 },    //0x4F
   { 0xF0, 0x88, 0x88, 0xF0, 0x80 ,0x80, 0x80, 0x00 },    //0x50
   { 0x70, 0x88, 0x88, 0x88, 0xA8 ,0x90, 0x68, 0x00 },    //0x51
   { 0xF0, 0x88, 0x88, 0xF0, 0xA0 ,0x90, 0x88, 0x00 },    //0x52
   { 0x78, 0x80, 0x80, 0x70, 0x08 ,0x08, 0xF0, 0x00 },    //0x53
   { 0xF8, 0x20, 0x20, 0x20, 0x20 ,0x20, 0x20, 0x00 },    //0x54
   { 0x88, 0x88, 0x88, 0x88, 0x88 ,0x88, 0x70, 0x00 },    //0x55
   { 0x88, 0x88, 0x88, 0x88, 0x88 ,0x50, 0x20, 0x00 },    //0x56
   { 0x88, 0x88, 0x88, 0xA8, 0xA8 ,0xA8, 0x50, 0x00 },    //0x57
   { 0x88, 0x88, 0x50, 0x20, 0x50 ,0x88, 0x88, 0x00 },    //0x58
   { 0x88, 0x88, 0x88, 0x50, 0x20 ,0x20, 0x20, 0x00 },    //0x59
   { 0xF8, 0x08, 0x10, 0x20, 0x40 ,0x80, 0xF8, 0x00 },    //0x5A
   { 0x70, 0x40, 0x40, 0x40, 0x40 ,0x40, 0x70, 0x00 },    //0x5B
   { 0x88, 0x50, 0xF8, 0x20, 0xF8 ,0x20, 0x20, 0x00 },    //0x5C
   { 0x70, 0x10, 0x10, 0x10, 0x10 ,0x10, 0x70, 0x00 },    //0x5D
   { 0x20, 0x50, 0x88, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x5E
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0xF8, 0x00 },    //0x5F
   { 0x40, 0x20, 0x10, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x60
   { 0x00, 0x00, 0x70, 0x08, 0x78 ,0x88, 0x78, 0x00 },    //0x61
   { 0x80, 0x80, 0xB0, 0xC8, 0x88 ,0x88, 0xF0, 0x00 },    //0x62
   { 0x00, 0x00, 0x70, 0x80, 0x80 ,0x88, 0x70, 0x00 },    //0x63
   { 0x08, 0x08, 0x68, 0x98, 0x88 ,0x88, 0x78, 0x00 },    //0x64
   { 0x00, 0x00, 0x70, 0x88, 0xF8 ,0x80, 0x70, 0x00 },    //0x65
   { 0x30, 0x48, 0x40, 0xE0, 0x40 ,0x40, 0x40, 0x00 },    //0x66
   { 0x00, 0x78, 0x88, 0x88, 0x78 ,0x08, 0x70, 0x00 },    //0x67
   { 0x80, 0x80, 0xB0, 0xC8, 0x88 ,0x88, 0x88, 0x00 },    //0x68
   { 0x20, 0x00, 0x60, 0x20, 0x20 ,0x20, 0x70, 0x00 },    //0x69
   { 0x10, 0x00, 0x30, 0x10, 0x10 ,0x90, 0x60, 0x00 },    //0x6A
   { 0x80, 0x80, 0x90, 0xA0, 0xC0 ,0xA0, 0x90, 0x00 },    //0x6B
   { 0x60, 0x20, 0x20, 0x20, 0x20 ,0x20, 0x70, 0x00 },    //0x6C
   { 0x00, 0x00, 0xD0, 0xA8, 0xA8 ,0x88, 0x88, 0x00 },    //0x6D
   { 0x00, 0x00, 0xB0, 0xC8, 0x88 ,0x88, 0x88, 0x00 },    //0x6E
   { 0x00, 0x00, 0x70, 0x88, 0x88 ,0x88, 0x70, 0x00 },    //0x6F
   { 0x00, 0x00, 0xF0, 0x88, 0xF0 ,0x80, 0x80, 0x00 },    //0x70
   { 0x00, 0x00, 0x68, 0x98, 0x78 ,0x08, 0x08, 0x00 },    //0x71
   { 0x00, 0x00, 0xB0, 0xC8, 0x80 ,0x80, 0x80, 0x00 },    //0x72
   { 0x00, 0x00, 0x70, 0x80, 0x70 ,0x08, 0xF0, 0x00 },    //0x73
   { 0x40, 0x40, 0xE0, 0x40, 0x40 ,0x48, 0x30, 0x00 },    //0x74
   { 0x00, 0x00, 0x88, 0x88, 0x88 ,0x98, 0x68, 0x00 },    //0x75
   { 0x00, 0x00, 0x88, 0x88, 0x88 ,0x50, 0x20, 0x00 },    //0x76
   { 0x00, 0x00, 0x88, 0x88, 0xA8 ,0xA8, 0x50, 0x00 },    //0x77
   { 0x00, 0x00, 0x88, 0x50, 0x20 ,0x50, 0x88, 0x00 },    //0x78
   { 0x00, 0x00, 0x88, 0x88, 0x78 ,0x08, 0x70, 0x00 },    //0x79
   { 0x00, 0x00, 0xF8, 0x10, 0x20 ,0x40, 0xF8, 0x00 },    //0x7A
   { 0x10, 0x20, 0x20, 0x40, 0x20 ,0x20, 0x10, 0x00 },    //0x7B
   { 0x20, 0x20, 0x20, 0x20, 0x20 ,0x20, 0x20, 0x00 },    //0x7C
   { 0x40, 0x20, 0x20, 0x10, 0x20 ,0x20, 0x40, 0x00 },    //0x7D
   { 0x00, 0x20, 0x10, 0xF8, 0x10 ,0x20, 0x00, 0x00 },    //0x7E
   { 0x00, 0x20, 0x40, 0xF8, 0x40 ,0x20, 0x00, 0x00 },    //0x7F
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x80
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x81
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x82
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x83
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x84
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x85
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x86
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x87
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x88
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x89
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x8A
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x8B
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x8C
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x8D
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x8E
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x8F
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x90
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x91
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x92
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x93
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x94
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x95
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x96
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x97
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x98
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x99
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x9A
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x9B
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x9C
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x9D
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x9E
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0x9F
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xA0
   { 0x00, 0x00, 0x00, 0x00, 0xE0 ,0xA0, 0xE0, 0x00 },    //0xA1
   { 0x38, 0x20, 0x20, 0x20, 0x00 ,0x00, 0x00, 0x00 },    //0xA2
   { 0x00, 0x00, 0x00, 0x20, 0x20 ,0x20, 0xE0, 0x00 },    //0xA3
   { 0x00, 0x00, 0x00, 0x00, 0x80 ,0x40, 0x20, 0x00 },    //0xA4
   { 0x00, 0x00, 0x00, 0x60, 0x60 ,0x00, 0x00, 0x00 },    //0xA5
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xA6  Tot hier ingevuld
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xA7
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xA8
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xA9
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xAA
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xAB
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xAC
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xAD
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xAE
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xAF
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB0
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB1
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB2
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB3
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB4
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB5
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB6
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB7
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB8
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xB9
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xBA
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xBB
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xBC
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xBD
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xBE
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xBF
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC0
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC1
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC2
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC3
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC4
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC5
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC6
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC7
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC8
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xC9
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xCA
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xCB
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xCC
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xCD
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xCE
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xCF
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD0
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD1
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD2
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD3
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD4
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD5
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD6
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD7
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD8
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xD9
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xDA
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xDB
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xDC
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xDD
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xDE
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xDF
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE0
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE1
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE2
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE3
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE4
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE5
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE6
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE7
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE8
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xE9
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xEA
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xEB
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xEC
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xED
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xEE
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xEF
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF0
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF1
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF2
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF3
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF4
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF5
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF6
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF7
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF8
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xF9
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xFA
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xFB
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xFC
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xFD
   { 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 },    //0xFE
   { 0xF8, 0xF8, 0xF8, 0xF8, 0xF8 ,0xF8, 0xF8, 0xF8 }     //0xFF
};

//----------------------------------------------------------------------------------------------------------------------------------

void LcdDisplayDrawChar(tagLcDisplay *display, int xpos, int ypos, char character)
{
  tagXlibContext *xc = display->xc;
  unsigned char *rows = CharacterMap[character];
  unsigned char  col;
  int ri,ci;
  int x;
 
  //Clear the digit before redrawing
  FillRect(xc, xpos, ypos, LcDisplayCharWidth, LcDisplayCharHeight, LcDisplayOffColor);

  //Each character has 8 rows and per row the top 5 bits are used
  for(ri=0;ri<8;ri++)
  {
    //Get the pixels for this row
    col = rows[ri];
    
    //Start at first dot
    x = xpos;
    
    //Plot the on pixels
    for(ci=0;ci<5;ci++)
    {
      //Check if current dot is on
      if(col & 0x80)
      {
        //Draw it if so
        FillRect(xc, x, ypos, LcDisplayDotSize, LcDisplayDotSize, LcDisplayOnColor);
      }
      
      //Select next dot
      col <<= 1;
      
      //Move to next dot
      x += LcDisplayDotSize;
    }
    
    //Move to next row
    ypos += LcDisplayDotSize;
  }
}

//----------------------------------------------------------------------------------------------------------------------------------
